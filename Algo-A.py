...
def getBatch(data, batchSize, current, entityVocab, literal_len, char_vocab):
    ...
    return hasNext, current+batchSize, dataPos[:,0], dataPos[:,1], dataPos[:,2], dataPos[:,3], pred_weight_pos, charPos, dataNeg[:,0], dataNeg[:,1], dataNeg[:,2], dataNeg[:,3], pred_weight_neg, charNeg, typePos_h, typePos_t, typeNeg_h, typeNeg_t
...
for line in file_lgd:
    elements = line.split(' ')
    s = codecs.decode(elements[0], 'unicode-escape')
    p = codecs.decode(elements[1], 'unicode-escape')
    o = codecs.decode(elements[2], 'unicode-escape')
...
with tfgraph.as_default():
    ...
    # input placeholder #
    pos_h = tf.placeholder(tf.int32, [None]) # [b]
    pos_t = tf.placeholder(tf.int32, [None]) # [b]
    pos_r = tf.placeholder(tf.int32, [None]) # [b]
    pos_r_trans = tf.placeholder(tf.int32, [None]) # [b]
    pos_c = tf.placeholder(tf.int32, [None, literal_len]) # [b, literal_len]
...
def run(graph, totalEpoch):
    ...
    with tf.Session(graph=graph) as session:
            ...
            for i in range(0, len(data)):
                ...
                while(hasNext and len(data[i]) > 0):
                    ...
                    if epoch % 2 == 0 and i == 0:
                        hasNext, current, ph, pr, pt, pr_trans, ppred, pc, nh, nr, nt, nr_trans, npred, nc, ph_type, pt_type, nh_type, nt_type = getBatch(data[i], batchSize, current, entity_vocab, literal_len, char_vocab)
                    else:
                        hasNext, current, ph, pr, pt, pr_trans, ppred, pc, nh, nr, nt, nr_trans, npred, nc, ph_type, pt_type, nh_type, nt_type = getBatch(data[i], batchSize, current, entity_vocab, literal_len, char_vocab)
                    feed_dict = {
                        pos_h: ph,
        ...
        np.savetxt("./save_data/TextKE_final_embedding_normalize_"+train_name+".txt", final_embeddings_normalized)
        np.savetxt("./save_data/TextKE_final_embedding_entity_"+train_name+".txt", final_embeddings_entity)
        np.savetxt("./save_data/TextKE_final_embedding_predicate_"+train_name+".txt", final_embeddings_predicate)
        cPickle.dump( reverse_entity_vocab, open( "./save_data/TextKE_dictionary_entity_"+train_name+".pickle", "wb" ) )
        cPickle.dump( reverse_predicate_vocab, open( "./save_data/TextKE_dictionary_predicate_"+train_name+".pickle", "wb" ) )
    writer.close()
    writer_tb.close()
